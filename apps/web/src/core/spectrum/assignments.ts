...
export function matchesAssignmentHit(
  assignment: PeakAssignment | null | undefined,
  hit: FormulaHit,
): boolean {
...
export function upsertAssignment(
  ...
  return [...assignments.filter((item) => item.peakId !== assignment.peakId), assignment].sort(
    (left, right) => left.mz - right.mz,
  );
}
