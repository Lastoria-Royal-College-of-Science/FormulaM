...
<section class="ui-card">
  ...
  {#if selectedPeak}
    <div class="grid grid-cols-2 gap-4 lt-md:grid-cols-1">
      <div class="rounded-2 border border-solid border-border bg-surface-2 px-3.5 py-3">
        ...
        <div class="mt-2 text-sm"><strong class="text-text">Observed m/z:</strong> {selectedPeak.mz.toFixed(6)}</div>
      ...
      <div class="rounded-2 border border-solid border-border bg-surface-2 px-3.5 py-3">
        ...
        {#if assignment}
          ...
          <div class="mt-1 text-sm"><strong class="text-text">Predicted m/z:</strong> {assignment.predictedMz?.toFixed(6) ?? "n/a"}</div>
  ...
  {:else}
    <p class="mb-0 text-sm text-muted">Click a peak in the spectrum plot to populate FormulaM m/z and prepare a formula assignment.</p>
  {/if}
</section>
