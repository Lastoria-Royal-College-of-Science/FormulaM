...
export const shortcuts = {
  ...
  "results-sort-button": `${transparentRestBorder} inline-flex w-full cursor-pointer items-center justify-between gap-2 rounded-[10px] border border-solid border-transparent bg-transparent px-1 py-1 text-left text-inherit shadow-none [appearance:none] transition-colors duration-150 hover:[border-color:var(--accent)] active:[border-color:var(--accent)] [&:focus:not(:hover):not(:active)]:[border-color:var(--control-rest-border)] [&:focus-visible:not(:hover):not(:active)]:[border-color:var(--control-rest-border)] hover:text-text active:text-text focus-visible:text-text focus-visible:shadow-control-glow ${noNativeOutline}`,
  ...
  "formula-table-head":
    "border-b border-border [border-bottom-style:solid] px-2 py-2 text-left text-[0.9rem] text-muted",
  "formula-table-cell": "px-2 py-2 align-middle",
  "chemical-formula": "whitespace-nowrap text-[0.95em]",
  "inline-code":
    "align-middle rounded-md border border-solid border-border bg-surface-2 px-1.5 py-0.5 font-mono text-[0.86em] text-text",
};
